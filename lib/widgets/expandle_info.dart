...
class _ExpandableInfoButtonState extends State<ExpandableInfoButton> {
  ...
  bool _localIsLongPressing = false;
  ...
  Widget build(BuildContext context) {
    ...
    // Determine if we should show mini circles
    final bool showMiniCircles = widget.isMobile
        ? (_localIsLongPressing || widget.isLongPressing)
        : _isExpanded;
    ...
    return GestureDetector(
      onLongPressStart: widget.isMobile
          ? (details) => _onLongPressStart()
          : null,
      onLongPressEnd: widget.isMobile ? (details) => _onLongPressEnd() : null,
      onTap: () {
        // Handle tap on mobile to show immediate feedback
        if (widget.isMobile && !showMiniCircles) {
          // Optionally show a hint that long press is available
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('Tap and hold to see options'),
              duration: const Duration(seconds: 2),
              behavior: SnackBarBehavior.floating,
            ),
          );
          ...
          child: Stack(
            ...
            children: [
              ...
              ...List.generate(_infoItems.length, (index) {
                ...
                final radius = showMiniCircles ? orbitRadius : 0.0;
                ...
                return AnimatedPositioned(
                  ...
                  left:
                      (orbitRadius + miniBallSize / 2) + x - (miniBallSize / 2),
                  top:
                      (orbitRadius + miniBallSize / 2) + y - (miniBallSize / 2),
                  child: AnimatedOpacity(
                    opacity: showMiniCircles ? 1.0 : 0.0,
                    duration: const Duration(milliseconds: 300),
                    child: AnimatedScale(
                      scale: showMiniCircles ? 1.0 : 0.3,
                      duration: const Duration(milliseconds: 300),
